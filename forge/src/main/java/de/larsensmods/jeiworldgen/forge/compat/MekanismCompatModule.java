                ...
                if(heightModifier != null){
                    ...
                    if(original.getType().equals(MekanismHeightProviderTypes.CONFIGURABLE.get())){
                        ...
                        switch(mekanismRange.minInclusive().anchorType().get()){
                            case ABSOLUTE -> minHeight = VerticalAnchor.absolute(mekanismRange.minInclusive().value().getAsInt());
                            case ABOVE_BOTTOM -> minHeight = VerticalAnchor.aboveBottom(mekanismRange.minInclusive().value().getAsInt());
                            case BELOW_TOP -> minHeight = VerticalAnchor.belowTop(mekanismRange.minInclusive().value().getAsInt());
                            default -> minHeight = VerticalAnchor.absolute(0);
                        }
                        switch(mekanismRange.maxInclusive().anchorType().get()){
                            case ABSOLUTE -> maxHeight = VerticalAnchor.absolute(mekanismRange.maxInclusive().value().getAsInt());
                            case ABOVE_BOTTOM -> maxHeight = VerticalAnchor.aboveBottom(mekanismRange.maxInclusive().value().getAsInt());
                            case BELOW_TOP -> maxHeight = VerticalAnchor.belowTop(mekanismRange.maxInclusive().value().getAsInt());
